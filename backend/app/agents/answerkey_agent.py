...
class AnswerKeyAgent:
    ...
    def _run(self, state: AgentState) -> dict[str, Any]:
        ...
        # ------------------------------------------------------------------
        # 3. Call LLM to generate model answers (using Llama 70B for large output space, with Qwen fallback)
        # ------------------------------------------------------------------
        raw_data = None
        content_context: str = state.get("content_context") or ""
        # Truncate content_context to 12000 characters to prevent TPM rate limits
        truncated_context = content_context[:12000]
        user_prompt = build_answerkey_user_prompt(
            validated_questions,
            content_context=truncated_context,
        )
        ...
        for idx, item in enumerate(raw_data):
            ...
            image_path = validated_src.get("image_path")
