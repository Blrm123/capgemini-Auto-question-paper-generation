...
from app.services.exam_validator import validate_exam_blueprint
...
class Orchestrator:
    ...
    def run(
        ...
        if final_state.get("topic_retrieval_debug"):
            rag_debug["topic_retrieval"] = final_state["topic_retrieval_debug"]
        ...
        # A printable examination must satisfy deterministic constraints.  The
        # LLM validation agent remains useful for quality feedback, but it is
        # not accepted as the final authority for counts, marks, or coverage.
        exam_validation = validate_exam_blueprint(
            questions=final_state.get("validated_questions", []),
            distribution=distribution,
            syllabus_topics=final_state.get("syllabus_topics", []),
            content_context=final_state.get("content_context", ""),
        )
        rag_debug["exam_validation"] = exam_validation

        # Log soft warnings (quality issues — do NOT block PDF generation)
        for w in exam_validation.get("warnings", []):
            logger.warning(f"Orchestrator: Exam quality warning — {w}")

        if not exam_validation["passed"]:
            elapsed = time.perf_counter() - start_time
            validation_errors = [f"Exam acceptance check: {error}" for error in exam_validation["errors"]]
            logger.error("Orchestrator: Deterministic exam acceptance check failed; PDFs were not generated.")
            ...
            return OrchestratorResult(
                ...
                final_pdf_path=None,
                answer_key_pdf_path=None,
                errors=accumulated_errors + validation_errors,
                elapsed_seconds=elapsed,
                final_state=final_state,
                rag_chunk_count=rag_chunk_count,
                debug_info=self._build_debug_info(rag_debug, final_state, uploaded_file_paths),
        ...
        logger.info("Orchestrator: Exam acceptance check PASSED. Proceeding to PDF generation.")
