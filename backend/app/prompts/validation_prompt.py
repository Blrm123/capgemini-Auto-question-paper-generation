...
8. Image/Diagram Preservation
   - If the retrieved source material contains available images (indicated by `[IMAGE AVAILABLE: path=...]` in chunk headers), verify that questions are tied to as many unique images as possible (aiming for multiple distinct image-based questions if multiple different diagrams are available in the notes).
   - Each diagram-based question MUST explicitly refer to its diagram in its text (e.g. "Identify the process shown in the diagram...", "Identify the compound/mineral in the image...", "Explain the dissociation shown in the illustration...").
   - If multiple images are available but only one (or none) is used, or if the questions don't explicitly refer to their images in their text, flag this as an issue, set "is_valid" to false, and modify or replace questions to include the correct `image_path` and reference the diagrams in their question text.

9. Dataset Table & Practical Problem Preservation
   - Do NOT remove, flatten, or replace dataset tables, sample matrices, or numerical calculation steps in practical questions.
   - Ensure practical dataset questions maintain their clean Markdown table formatting and explicit numerical calculation instructions.
...
- In "validated_questions", return the SAME questions with minor wording fixes only.

## CRITICAL RULES (MUST FOLLOW)
- You MUST return EXACTLY the same number of questions you received. DO NOT add new questions. DO NOT remove questions.
- You MUST preserve each question's id, marks, and difficulty EXACTLY as given.
- You MUST NOT change the marks value of any question.
- You MAY only improve question wording, fix grammar, or clarify ambiguity.
- If coverage gaps exist, report them in "issues_found" but DO NOT invent new questions to fill them.
