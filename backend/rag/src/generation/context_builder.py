...
def document_to_chunk_dict(doc: Document) -> Dict[str, Any]:
    ...
    return {
        ...
        "image_explanation": doc.metadata.get("image_explanation"),
        "image_id": doc.metadata.get("image_id"),
    }



def format_chunks_for_prompt(chunks: List[Dict[str, Any]], exclude_images: bool = False) -> str:
    ...
    for chunk in chunks:
        ...
        if image_path and not exclude_images:
            header += f"\n[IMAGE AVAILABLE: path={image_path}]"
            
        if exclude_images:
            # Strip any inline [Image Description: ...] blocks to save tokens
